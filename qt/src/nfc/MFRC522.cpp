/*
* MFRC522.cpp - Library to use RFID MODULE KIT 13.56 MHZ WITH TAGS on RPi
...
/////////////////////////////////////////////////////////////////////////////////////
// Functions for setting up the RPi
/////////////////////////////////////////////////////////////////////////////////////
 ...
 */
MFRC522::MFRC522( Comm* comm):MFRC522(comm,UINT8_MAX) {
 ...
 */
MFRC522::MFRC522( Comm* comm,
                  byte resetPowerDownPin	///< RPi pin connected to MFRC522's reset and power down input (Pin 6, NRSTPD, active low). If there is no connection from the CPU to NRSTPD, set this to UINT8_MAX. In this case, only soft reset will be used in PCD_Init().
                  ) {
    PCDComm = comm;
...
void MFRC522::PCD_WriteRegister( PCD_Register reg,	///< The register to write to. One of the PCD_Register enums.
    ...
    PCDComm->writeBytes(reg,1,&value);
    return;
...
void MFRC522::PCD_WriteRegister(	PCD_Register reg,	///< The register to write to. One of the PCD_Register enums.
    ...
    PCDComm->writeBytes(reg,count,values);
    return;
} // End PCD_WriteRegister()
...
void MFRC522::PCD_ReadRegister(	PCD_Register reg,	///< The register to read from. One of the PCD_Register enums.
    ...
    PCDComm->readBytes(reg,count,values,rxAlign);
    return;
} // End PCD_ReadRegister()
...
MFRC522::StatusCode MFRC522::PCD_CalculateCRC(	byte *data,		///< In: Pointer to the data to transfer to the FIFO for CRC calculation.
    ...
    // TODO check/modify for other architectures
...
void MFRC522::PCD_Init() {
    ...
    // Initialize communication to the PCD
    PCDComm->initComm();
 ...
 * Initializes the MFRC522 chip.
 */
void MFRC522::PCD_Init(	byte resetPowerDownPin	///< RPi pin connected to MFRC522's reset and power down input (Pin 6, NRSTPD, active low)
                        ) {
    _resetPowerDownPin = resetPowerDownPin;
    PCD_Init();
} // End PCD_Init()
...
void MFRC522::PCD_SoftPowerUp(){
    ...
    usleep(500000); // workaround wait 500 ms => TODO
    /***TODO
...
MFRC522::StatusCode MFRC522::PCD_CommunicateWithPICC(	byte command,		///< The command to execute. One of the PCD_Command enums.
    ...
    //printf("PCD_CommunicateWithPICC ... irq 0x%x len %d command 0x%x backlen %d\n",waitIRq,sendLen,command,*backLen);
    ...
    for (i = 2000; i > 0; i--) {
        ...
        //printf("n %x waitIrq %x ",n,waitIRq);
    ...
    //printf("PCD_CommunicateWithPICC ... no timeout\n");
...
void MFRC522::PICC_DumpMifareClassicSectorToSerial(Uid *uid,			///< Pointer to Uid struct returned from a successful PICC_Select().
    ...
    for (int8_t blockOffset = no_of_blocks - 1; blockOffset >= 0; blockOffset--) {
        ...
        if (firstInGroup) {
            // Print access bits
            printf(" [ %i %i %i ] ",(g[group] >> 2) & 1,(g[group] >> 1) & 1,(g[group] >> 0) & 1);
            if (invertedError)
